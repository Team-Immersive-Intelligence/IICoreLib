...
import net.iiteam.corelib.Color;
import net.iiteam.corelib.EnumUtil;
import net.iiteam.corelib.Utils;
...
public class EasyNBT extends Constants.NBT
{
	 ...
	 * @param key name of this tag
	 */
	public EasyNBT withColor(String key, Color color)
	{
	...
	public EasyNBT withAny(String name, @Nullable Object value)
	{
		...
		if(value instanceof Color)
			return withColor(name, (Color)value);
	 ...
	 * @param key name of this tag
	 */
	public BlockPos getPos(String key) throws Exception
	{
		...
		if(wrapped.hasKey(key))
			throw new Exception("Malformed BlockPos tag for \""+key+"\" in"+Arrays.toString(new Throwable().getStackTrace()));
	 ...
	 * @param key name of this tag
	 */
	public DimensionBlockPos getDimPos(String key) throws Exception
	{
		int[] pos = getIntArray(key);
		if(pos.length > 3)
		...
		//only if key is present
		if(wrapped.hasKey(key))
			throw new Exception("Malformed DimensionBlockPos tag for \""+key+"\" in"+Arrays.toString(new Throwable().getStackTrace()));
	 ...
	 * @param key name of this tag
	 */
	public Vec3d getVec3d(String key) throws Exception
	{
		...
		//only if key is present
		if(wrapped.hasKey(key))
			throw new Exception("Malformed Vec3D tag for \""+key+"\" in"+Arrays.toString(new Throwable().getStackTrace()));
	 ...
	 * @param key name of this tag
	 */
	public Vector3f getVector3f(String key) throws Exception
	{
		NBTTagList pos = getList(key, TAG_DOUBLE);
		...
		//only if key is present
		if(wrapped.hasKey(key))
			throw new Exception("Malformed Vec3f tag for \""+key+"\" in"+Arrays.toString(new Throwable().getStackTrace()));
	 ...
	 * @param key name of this tag
	 */
	public Vector2f getVector2f(String key) throws Exception
	{
		...
		//only if key is present
		if(wrapped.hasKey(key))
			throw new Exception("Malformed Vec2f tag for \""+key+"\" in"+Arrays.toString(new Throwable().getStackTrace()));
	...
	{
		return EnumUtil.enumValue(type, getString(key));
	}

	public Color getColor(String key)
	{
		return Color.fromPackedARGB(getInt(key));
	}
	...
	public EasyNBT checkSetColor(String key, Consumer<Color> ifPresent)
	{
		if(wrapped.hasKey(key))
